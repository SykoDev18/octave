...
plot(Pt(1,:), Pt(2,:), "-+b", "LineWidth", 1);     # Trasladada en azul
plot(Pr(1,:), Pr(2,:), "--*r", "LineWidth", 1);    # Transformada final en rojo dashed

dx = 20; dy = 20;
axis([-dx dx -dy dy]);
title("Flor - ROTACION-TRASLACION 2D (TRP)");
text(0, 5, "Imagen ORIGINAL");
text(-15, 5, "Imagen TRANSFORMADA");
text(-5, -12, "Marco Antonio Miranda Muñoz", "fontsize", 22, "color", "b");
# Autor: Marco Antonio Miranda Muñoz
# Ejes
plot([-dx dx],[0 0], "-+m");
plot([0 0],[-dy dy], "-+m");
